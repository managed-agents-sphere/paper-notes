...
| 维度 | 取值 |
|---|---|
| 最小可缓存长度 | 4,096 tokens（Opus 4.5+ / Haiku 4.5）；2,048（Sonnet 4.6）；1,024（Sonnet 4.5 / Opus 4.0/4.1 / Sonnet 4.0） |
...
### 2.4 cache_control 挂在哪

**只认顶层 block**——`tools` 定义、`system` 内容、`messages.content` 里的 text / image / document / tool_use / tool_result，user 与 assistant 双向。

**子结构挂不上**：`thinking`、citation 节点、空 text block。其中 `thinking` 与 citation 仍能进 cache，靠**包它的顶层 block 顺带**——`thinking` 跟随前置 assistant turn 一起缓存（命中时按 `input_tokens` 计费，不免费）；citation 把 marker 挂在引用源的 `document` 上。

### 2.5 失效级联：tools → system → messages

层级硬规则——**上层动则下层连带**，下层动不波及上层。

| 改动 | tools | system | messages |
|---|---|---|---|
| 改 tool 定义（name / desc / params） | ✘ | ✘ | ✘ |
| 切 web search 开关 | ✓ | ✘ | ✘ |
| 切 citations 开关 | ✓ | ✘ | ✘ |
| `speed: "fast"` ↔ 标准速度 | ✓ | ✘ | ✘ |
| 改 `tool_choice` | ✓ | ✓ | ✘ |
| 任意位置增删 image | ✓ | ✓ | ✘ |
| thinking 参数（开关 / budget） | ✓ | ✓ | ✘ |
| 给带 thinking 的请求加 non-tool-result user 块 | ✓ | ✓ | 模型特异：Opus 4.5+ / Sonnet 4.6+ ✓；其他模型 strip thinking → ✘ |

工程结论：把**最稳定**的内容塞最前；breakpoint 永远落在"自此往后会变"那条线**之前**。

### 2.6 token 账本与可观测性

**三个 `usage` 字段**：

- `cache_creation_input_tokens` —— 这次写进 cache 的 token
- `cache_read_input_tokens` —— 这次从 cache 读的 token
- `input_tokens` —— **只算最后 breakpoint 之后**那段没参与 cache 的 token（容易让人误判输入总量）

**总量公式**：`total_input = read + creation + input`


### 2.8 1h TTL 与跨 TTL 混用

**何时上 1h（不是默认）**：

- side agent 跑 >5min
- 多轮对话间隔常超 5min
- 想吃 rate limit 优势：cache 命中不计 rate limit，1h 命中窗大

**何时仍是 5min**：节奏 ≤5min 的工作流——5min 命中刷新免费，没必要付 2× write 价

**写法**：`"cache_control": {"type": "ephemeral", "ttl": "1h"}`

**响应里多一项**：`usage.cache_creation` 拆分为 `ephemeral_5m_input_tokens` / `ephemeral_1h_input_tokens`，原 `cache_creation_input_tokens` 等于二者之和。

**混用硬约束**：**长 TTL 必须排在短 TTL 之前**——1h block 必须出现在所有 5min block 之前，反过来 API 会拒。

**A / B / C 三点账单**（混用时）：

- **A** = 命中点最深处的 token 数（无命中 = 0）
- **B** = A 之后第一个 1h breakpoint 的 token 数（无 1h block = A）
- **C** = 最末一个 breakpoint 的 token 数
...
| 维度 | 规则 |
|---|---|
| 命中条件 | prefix 100% 字节一致（含到 `cache_control` 当前 block） |
| 隔离粒度（Claude API + Azure，**2026-02-05 起**） | **workspace 级** |
| 隔离粒度（Bedrock + Vertex） | 仍 **org 级** |
| 跨 org | 永不共享 |
| 对输出影响 | 完全无——response 与不开 cache 时一致 |

> 迁移提醒：2026-02-05 之后 Claude API 由 org-level → workspace-level 隔离。如果产品形态此前依赖"同 org 跨 workspace 共享 cache"省钱，那条路会断；多 workspace 部署需要重新评估命中率假设。

### 2.10 排错速查

| 症状 | 多半是 |
|---|---|
| `creation == 0` 且 `read == 0` | prefix 没过最小阈值（静默失败） |
| 第一次写后 5min 内再发还 miss | breakpoint 放在了**会变**的那个 block 上（timestamp / per-request 字段 / 用户输入） |
| 偶尔 miss 偶尔 hit | tool_use 的 JSON key 顺序不稳——Swift / Go 默认会随机化，自己强制 stable ordering |
| 持续命中后突然全失效 | 上层动了：tool 定义变 / 切 web search / 切 citations / 改 `tool_choice` / 加图 / 改 thinking 参数 |
| Opus 4.5 之前的模型，agent 加自然语言追问后 cache 全没 | thinking block 被 strip 引发的级联失效（参 §2.7） |

**Breakpoint 放置铁律**：永远放在"跨请求**保持不变**"的最后一个 block 上；不是放在"会变"的那个块上。lookback 不会越过 breakpoint 找稳定 prefix——breakpoint 之前的内容只有它自己被前次写过才能命中。
